...
### EXAMPLES C LIB FUNCTIONS :
| Function   | Always system call? | Explanation                                                                                         |
| ---------- | ------------------: | --------------------------------------------------------------------------------------------------- |
| `strlen()` |                   ❌ | Simply counts characters in memory. No kernel service is needed.                                    |
| `sleep()`  |                   ✅ | The process must ask the OS/kernel to suspend its execution.                                        |
| `malloc()` |                   ❌ | The C library may already have free heap memory available, so it doesn't always need a system call. |
| `exit()`   |                   ✅ | Ultimately the process must inform the kernel that it is terminating.                               |

> malloc at first uses system call then does require for subsequet operations so not always.
